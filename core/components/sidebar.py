...
class Sidebar(ft.Container):
    def __init__(self, active_section="personal", on_navigate=None, on_logout=None, on_toggle_theme=None, dark_mode=True):
        super().__init__()
    ...
    def _build_item_button(self, section, icon, label, tc):
        ...
        btn = ft.Container(
            content=ft.Row([
                ft.Icon(
                    icon,
                    color=ft.Colors.GREEN_400 if is_active else tc["sidebar_icon"],
                    size=22,
                ),
                ft.Text(
                    label,
                    color=ft.Colors.GREEN_400 if is_active else tc["sidebar_text"],
                    size=14,
    ...
    def _rebuild_items(self):
        ...
        for key, btn in self._item_buttons.items():
            ...
            btn.content.controls[0].color = ft.Colors.GREEN_400 if is_active else tc["sidebar_icon"]
            btn.content.controls[1].color = ft.Colors.GREEN_400 if is_active else tc["sidebar_text"]
            btn.content.controls[1].weight = ft.FontWeight.W_600 if is_active else ft.FontWeight.W_500
